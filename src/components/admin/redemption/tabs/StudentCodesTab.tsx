...
interface StudentCodesTabProps {
...
const StudentCodesTab: React.FC<StudentCodesTabProps> = ({
  codes,
  selectedCodes,
  allSelected,
  isLoading,
  ...
  const filteredCodes = codes.filter(code => code.type === 'student');
  
  if (isLoading) {
    return <div className="p-6 text-center">Loading student codes...</div>;
  }
  
  if (filteredCodes.length === 0) {
    return (
      <div className="p-8 text-center">
        <p className="text-muted-foreground mb-4">No student redemption codes found.</p>
  ...
  return (
    <div className="p-4">
      <div className="mb-4">
        <h3 className="text-lg font-medium">Student Redemption Codes</h3>
        <p className="text-sm text-muted-foreground">
          These codes can be distributed to students for platform access.
        </p>
      </div>
...
export default StudentCodesTab;
