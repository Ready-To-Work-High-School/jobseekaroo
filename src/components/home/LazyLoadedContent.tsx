...
const LazyLoadedContent = () => {
  return (
    <>
      ...
      {/* Lazy load recommendations section with error boundary */}
      <Suspense fallback={<div className="py-8 flex justify-center"><div className="w-8 h-8 border-4 border-primary/30 border-t-primary rounded-full animate-spin"></div></div>}>
        <ErrorBoundary>
          <UserRecommendationsSection />
        </ErrorBoundary>
      </Suspense>
      
      {/* Lazy load chatbot with error boundary */}
