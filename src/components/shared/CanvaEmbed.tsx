...
const CanvaEmbed: React.FC<CanvaEmbedProps> = ({
  ...
  return <div className={`w-full ${className}`}>
      <div className="relative w-full overflow-hidden rounded-lg shadow-lg transition-all duration-300 hover:shadow-xl">
        {!isLoaded && !isError && <div className="absolute inset-0 flex items-center justify-center bg-gray-100/80 z-10">
            <div className="w-12 h-12 border-4 border-primary/30 border-t-primary rounded-full animate-spin"></div>
          </div>}
        
        {isError && <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-100 z-10 p-4 text-center">
            ...
            <a href={shareUrl} target="_blank" rel="noopener noreferrer" className="mt-2 text-sm text-blue-600 hover:underline">
        ...
        willChange: "transform"
          ...
          <iframe loading="lazy" onLoad={handleIframeLoad} onError={handleIframeError} className={`absolute top-0 left-0 w-full h-full border-0 p-0 m-0 transition-opacity duration-500 ${isLoaded ? 'opacity-100' : 'opacity-0'}`} src={embedUrl.replace('designId', designId)} allowFullScreen={true} allow="fullscreen" title={designName} />
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between mt-3 mb-2 px-2">
          <div className="flex items-center">
            ...
            {(designName || authorName) && <div className="flex items-center">
                {designName && <a href={shareUrl.replace(/designId/g, designId)} target="_blank" rel="noopener noreferrer" className="text-sm text-gray-600 hover:text-primary transition-colors flex items-center gap-1">
                ...
                {designName && authorName && <span className="mx-2 text-gray-400">•</span>}
                
                {authorName && <span className="text-sm text-gray-500">by {authorName}</span>}
              </div>}
          </div>
        </div>
      </div>
